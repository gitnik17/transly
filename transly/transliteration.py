...
def train(model_path='./trained_model/', model_file_name='model.h5', training_data_path='./train.csv'):
    """
...
def load_model(model_path=filepath+'/trained_models/hi2en/', model_file_name='model.h5'):
    """
    loads a pre-trained word boundary model
    :return: Seq2Seq object
    """
    model_path = filepath+'/trained_models/{}/'.format(model_path) if model_path in ['en2hi', 'hi2en'] else model_path
